...
func populateProduct(db *sql.DB) {
	...
	_, err := db.Exec(query,
		"1", "TechBrave X100 Smartphone", "High-end smartphone with AI features.", "https://example.com/img1.jpg", 8999.99, 100, nil, "Mobile Phones & Tablets",
...
func populateMember(db *sql.DB) {
	...
	_, err := db.Exec(query,
		"0", "Gold", time.Now().Format(time.DateTime),
		"1", "Silver", time.Now().Format(time.DateTime),
		"3", "Platinum", time.Now().Format(time.DateTime),
	)
	if err != nil {
